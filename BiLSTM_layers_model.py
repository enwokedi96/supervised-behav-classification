# DEEP cONV NET USING OPTIMAL N-KERNEL CONV3D LAYERS WITH INTERMEDIATE DENSE LAYER FEATURES CONCAT'ED (NEW BILSTM)
# - INTER-LAYER SHARING/CONNECTIONS BETWEEN DUAL STREAMS
# LESS DROPOUTS THAN PREVIOUS NETS
# RECORDED ACC ON CLIPPED DATA: 74.97
...
le = LabelEncoder()
choice = '3d'
temporal_length,img_rows, img_cols, colors = 8,128,128,3
...
def AE_model_3_4_5(fusiontype='late_fusion', method='concat'): #not working yet..........................
  input_shape = (temporal_length, img_rows, img_cols, colors)
  input_shape_2 = (temporal_length, img_rows, img_cols, colors)
  input = Input(shape=input_shape)
  input_2 = Input(shape=input_shape_2)
  scale_, offset = 1./255., 0
  # method 1: late fusion of multi kernel network
...
###################################################-----###############################################################################################
model = AE_model_3_4(fusiontype='late_fusion', method='concat'); name='supervised_3d_conv.h5'; bs = 32
##############################################====######################################################################
